...
const EditProofModal = ({ isOpen, onClose, onSave, proof }) => {
  const { user } = useAuth();
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onSave(proof.id, { ...formData, type, author: user?.username || 'Agente Federal', authorId: user?.id });
    // Reset
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[calc(100vh-2rem)] shadow-2xl overflow-hidden animate-fade-in-up flex flex-col">
        
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 bg-slate-900/50">
          <h3 className="text-lg font-bold text-white">Editar Prova</h3>
