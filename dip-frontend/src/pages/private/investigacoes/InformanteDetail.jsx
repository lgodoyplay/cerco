...
import { ArrowLeft, Plus, FolderOpen, Upload, FileText, Trash2, Edit3, Download, CheckCircle, AlertCircle, X, Clock, User, Phone, Mail, MapPin, Tag, Filter, Search, Calendar, Flag, FolderPlus, Clipboard, Shield, AlertTriangle, Info, Star, Eye, RefreshCw } from 'lucide-react';
...
const InformanteDetail = () => {
  ...
  const { user } = useAuth();
  const { getInformante, updateInformante, deleteInformante, addEntry, updateEntry, deleteEntry, addEntryFile, removeEntryFile, CATEGORIES, PRIORITIES, ENTRY_STATUSES } = useInformantes();

  const [informante, setInformante] = useState(null);
  const [loading, setLoading] = useState(true);
  const [notification, setNotification] = useState(location.state?.notification || null);
  const [confirmDelete, setConfirmDelete] = useState(null);
  const [confirmEntryDelete, setConfirmEntryDelete] = useState(null);
  ...
  const filteredEntries = (informante?.entries || []).filter(entry => {
  ...
  const stats = {
    total: (informante?.entries || []).length,
    byCategory: {},
    byStatus: {},
    byPriority: {},
    pending: (informante?.entries || []).filter(e => e.status === 'Pendente').length,
    reviewed: (informante?.entries || []).filter(e => e.status === 'Revisado').length,
    verified: (informante?.entries || []).filter(e => e.status === 'Verificado').length,
  };

  (informante?.entries || []).forEach(entry => {
  ...
  const handleAddEntry = () => {
    ...
    setInformante(prev => prev ? {
      ...prev,
      entries: [entryData, ...(prev.entries || [])],
  ...
  const handleUpdateEntry = () => {
    ...
    updateEntry(id, editingEntry.id, entryData);
    setInformante(prev => prev ? {
      ...prev,
  ...
  const removeFileFromEntry = (fileId) => {
    ...
    <div className="max-w-7xl mx-auto pb-20 space-y-6">
      ...
      {confirmEntryDelete && (
        <ConfirmModal
          ...
          onConfirm={() => handleDeleteEntry(confirmEntryDelete)}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8 shadow-xl relative overflow-hidden">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 relative z-10">
          <div>
            ...
            {informante.apelido && (
              <p className="text-slate-400 text-lg">Alias: {informante.apelido}</p>
            )}
            {informante.relacao && (
              <p className="text-slate-500 text-sm mt-1">Relação: {informante.relacao}</p>
            )}
          </div>
        ...
        {/* Quick Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-6 border-t border-slate-800 relative z-10">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-white flex items-center gap-2">
            <Plus className="text-federal-500" size={24} />
          ...
          >
            <Plus size={18} /> {isAddingEntry ? 'Cancelar' : 'Nova Informação'}
          </button>
        </div>
          ...
          <div className="mb-6 p-4 bg-slate-800/50 border border-slate-700 rounded-xl space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-slate-900 border border-slate-700 rounded-lg px-4 py-2.5 text-white placeholder-slate-600 focus:border-federal-500 focus:ring-1 focus:ring-federal-500 outline-none transition-all resize-none"
              />
            </div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {entryFiles.map((f, idx) => (
                    <div key={idx} className="flex items-center gap-2 bg-slate-900 border border-slate-700 rounded-lg px-3 py-1.5 text-xs text-slate-300">
                      ...
                      <button onClick={() => removeFileFromEntry(f.id)} className="text-red-400 hover:text-red-300 ml-1" aria-label="Remover arquivo">
                        <X size={12} />
                      </button>
                    </div>
                  ))}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 md:p-8">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-slate-400">{filteredEntries.length} de {(informante?.entries || []).length}</span>
          </div>
        </div>
                ...
                <div key={entry.id} className="bg-slate-800/50 border border-slate-700 rounded-xl p-5 hover:border-slate-600 transition-colors">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex gap-1 shrink-0">
                      ...
                      <button
                        onClick={() => setConfirmEntryDelete(entry.id)}
