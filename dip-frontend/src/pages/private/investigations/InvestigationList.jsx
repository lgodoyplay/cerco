...
import { Search, Plus, FileText, Clock, CheckCircle, AlertTriangle, FolderOpen, Archive, Edit3, Trash2, X, Filter } from 'lucide-react';
...
const InvestigationList = ({ category = 'criminal', title }) => {
  ...
  const activeCount = categoryInvestigations.filter(inv => inv.status !== 'Encerrada' && inv.status !== 'Finalizada').length;
  const closedCount = categoryInvestigations.filter(inv => inv.status === 'Encerrada' || inv.status === 'Finalizada').length;

  const filteredInvestigations = categoryInvestigations.filter(inv => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      ...
      setNotification({ type: 'success', message: 'Investigação deletada com sucesso.' });
    } catch (error) {
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        ...
        <div className="flex items-center gap-2 bg-slate-900 border border-slate-800 rounded-xl px-3">
          ...
          <select
            ...
            className="bg-transparent text-slate-300 text-sm py-2.5 outline-none cursor-pointer"
          >
        ...
        {filteredInvestigations.length === 0 ? (
          ...
          filteredInvestigations.map(inv => (
            <div key={inv.id} className="bg-slate-900 border border-slate-800 hover:border-federal-500/50 rounded-2xl p-6 transition-all hover:shadow-xl hover:shadow-federal-900/10 flex flex-col">
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="flex gap-2">
                  {canManage && (
                    <>
                      <button
                        onClick={(e) => { e.stopPropagation(); navigate(category === 'financial' ? `/dashboard/revenue/investigations/${inv.id}/edit` : `/dashboard/investigations/${inv.id}/edit`); }}
              ...
              <Link to={category === 'financial' ? `/dashboard/revenue/investigations/${inv.id}` : `/dashboard/investigations/${inv.id}`} className="flex-1">
                <h3 className="text-xl font-bold text-white mb-2 hover:text-federal-400 transition-colors line-clamp-1">{inv.title}</h3>
                <p className="text-slate-400 text-sm line-clamp-2 mb-4">{inv.description}</p>
              </Link>
              ...
              <div className="mt-auto space-y-2 pt-4 border-t border-slate-800">
                <div className="flex items-center justify-between text-xs text-slate-500">
                  ...
                  <div className="flex items-center gap-1.5">
                    <FileText size={13} />
                    <span>{inv.proofs?.length || 0} prova{inv.proofs?.length !== 1 ? 's' : ''}</span>
                  </div>
                </div>
                {inv.investigator?.nome && (
                  <div className="text-xs text-slate-500 truncate">
                    Responsável: <span className="text-slate-400">{inv.investigator.nome}</span>
                  </div>
                )}
                <Link
