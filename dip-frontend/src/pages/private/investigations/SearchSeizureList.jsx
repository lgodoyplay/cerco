...
import { Search, Plus, FileText, Clock, Archive, Edit3, Trash2, Home, Car, AlertTriangle, CheckCircle, X } from 'lucide-react';
...
const SearchSeizureList = () => {
  ...
  const allSeizures = investigations.filter(inv => inv.category === 'search_and_seizure');
  const activeCount = allSeizures.filter(inv => inv.status !== 'Encerrada' && inv.status !== 'Finalizada' && inv.status !== 'Arquivada').length;
  const closedCount = allSeizures.filter(inv => inv.status === 'Encerrada' || inv.status === 'Finalizada' || inv.status === 'Arquivada').length;

  const filtered = allSeizures.filter(inv => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      await deleteInvestigation(confirmDelete);
      setNotification({ type: 'success', message: 'Busca e apreensão deletada com sucesso.' });
    } catch (error) {
  ...
  return (
    <div className="max-w-7xl mx-auto pb-10">
      {confirmDelete && (
        <ConfirmModal
          message="Tem certeza que deseja deletar esta busca e apreensão? Esta ação não pode ser desfeita."
      ...
      {notification && (
        <div className={clsx(
          ...
          <button onClick={() => setNotification(null)} className="text-current/80 hover:text-current transition-colors">
            <X size={18} />
          </button>
        </div>
      )}
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          <h2 className="text-3xl font-bold text-white">Busca e Apreensão</h2>
        ...
        {canManage && (
          ...
          >
            <Plus size={20} /> Nova Busca e Apreensão
          </Link>
        )}
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-8">
        <div className="flex bg-slate-900 p-1 rounded-xl border border-slate-800">
          ...
          <button
            ...
            className={clsx("px-5 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2", filter === 'closed' ? "bg-slate-800 text-white shadow" : "text-slate-400 hover:text-slate-200")}
          >
            <Archive size={16} /> Arquivadas
            <span className={clsx("ml-1 px-1.5 py-0.5 rounded text-xs", filter === 'closed' ? "bg-slate-600 text-white" : "bg-slate-700 text-slate-400")}>{closedCount}</span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.length === 0 ? (
          ...
          filtered.map(inv => (
            <div key={inv.id} className="bg-slate-900 border border-slate-800 hover:border-federal-500/50 rounded-2xl p-6 transition-all hover:shadow-xl hover:shadow-federal-900/10 flex flex-col">
              <div className="flex justify-between items-start mb-4">
                ...
                {canManage && (
                  <div className="flex gap-2">
                    <button
                      onClick={(e) => { e.stopPropagation(); navigate(`/dashboard/search-seizure/${inv.id}/edit`); }}
              ...
              <Link to={`/dashboard/search-seizure/${inv.id}`} className="flex-1">
                <h3 className="text-xl font-bold text-white mb-1 hover:text-federal-400 transition-colors line-clamp-1">{inv.title}</h3>
                {inv.nomeEntidade && <p className="text-slate-400 text-sm mb-2">{inv.nomeEntidade}</p>}
                <div className="flex items-center gap-3 text-slate-500 text-xs">
                  <span className="flex items-center gap-1"><Home size={13} /> {inv.quantidadeCasas} casa{inv.quantidadeCasas !== 1 ? 's' : ''}</span>
                  <span className="flex items-center gap-1"><Car size={13} /> {inv.quantidadeCarros} carro{inv.quantidadeCarros !== 1 ? 's' : ''}</span>
                </div>
              </Link>

              <div className="flex items-center justify-between pt-4 border-t border-slate-800 mt-4">
                <span className="text-slate-500 text-xs">{new Date(inv.createdAt).toLocaleDateString('pt-BR')}</span>
                <Link to={`/dashboard/search-seizure/${inv.id}`} className="flex items-center gap-1 text-federal-400 text-xs font-bold uppercase tracking-wider hover:underline">
                  Abrir →
                </Link>
              </div>
            </div>
