...
import {
  ArrowLeft, Plus, Lock, Download, Clock, Shield, User, Users, FileText,
  CheckCircle, AlertCircle, X, Edit3, Trash2, Building2
} from 'lucide-react';
...
const InvestigationDetail = () => {
  ...
  const loadInvestigation = useCallback(async () => {
    ...
    try {
      ...
      if (!data) { navigate(backLink); return; }
      setInvestigation(data);
    } catch {
      setNotification({ type: 'error', message: 'Não foi possível carregar a investigação.' });
    } finally {
      setLoading(false);
    }
  }, [id, getInvestigation, navigate, backLink]);
  ...
  const handleAddProof = async (proofData) => {
    try {
      ...
      setNotification({ type: 'success', message: 'Prova adicionada com sucesso.' });
    } catch (error) {
      setNotification({ type: 'error', message: error?.message || 'Não foi possível adicionar a prova.' });
    }
  };
  ...
  const handleEditProof = async (proofId, proofData) => {
    try {
      ...
      setProofToEdit(null);
      setNotification({ type: 'success', message: 'Prova atualizada com sucesso.' });
    } catch (error) {
      setNotification({ type: 'error', message: error?.message || 'Não foi possível atualizar a prova.' });
    }
  };
    ...
    setConfirmAction({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          setInvestigation(prev => ({ ...prev, proofs: prev.proofs.filter(p => p.id !== proofId) }));
          setNotification({ type: 'success', message: 'Prova removida com sucesso.' });
        } catch (error) {
          setNotification({ type: 'error', message: error?.message || 'Não foi possível remover a prova.' });
        }
      }
    });
  };
  ...
  const handleFinalize = () => {
    setConfirmAction({
      message: 'Tem certeza que deseja finalizar esta investigação? Esta ação não pode ser desfeita.',
      onConfirm: async () => {
        setConfirmAction(null);
        setIsGeneratingPdf(true);
        try {
          const updatedInv = await closeInvestigation(id);
          setInvestigation(updatedInv);
          await generateProfessionalPDF(
            updatedInv, user, templates?.investigation, 'investigation',
            templates?.__layoutConfig?.investigation, templates?.__pageHeaderConfig,
            {}, templates?.__investigationCoverTemplate, templates?.__investigationNumberConfig
          );
          setNotification({ type: 'success', message: 'Investigação finalizada e relatório gerado com sucesso.' });
        } catch (error) {
          setNotification({ type: 'error', message: error?.message || 'Não foi possível finalizar a investigação.' });
        } finally {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      await generateProfessionalPDF(
        investigation, user, templates?.investigation, 'investigation',
        templates?.__layoutConfig?.investigation, templates?.__pageHeaderConfig,
        {}, templates?.__investigationCoverTemplate, templates?.__investigationNumberConfig
      );
      setNotification({ type: 'success', message: 'Relatório gerado com sucesso.' });
    } catch (error) {
      setNotification({ type: 'error', message: error?.message || 'Não foi possível gerar o relatório.' });
    } finally {
      setIsGeneratingPdf(false);
    }
  };
  ...
  const handleDeleteInvestigation = () => {
    setConfirmAction({
      message: 'Tem certeza que deseja deletar esta investigação? Esta ação não pode ser desfeita.',
      onConfirm: async () => {
        setConfirmAction(null);
        try {
          await deleteInvestigation(id);
          navigate(backLink, { state: { notification: { type: 'success', message: 'Investigação deletada com sucesso.' } } });
        } catch (error) {
          setNotification({ type: 'error', message: error?.message || 'Não foi possível deletar a investigação.' });
        }
      }
    });
  };
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 relative z-10">
          <div>
            <div className="flex items-center gap-3 mb-2">
              <span className={clsx(
                ...
                isClosed ? "bg-red-500/10 text-red-400 border-red-500/20" : "bg-emerald-500/10 text-emerald-400 border-emerald-500/20"
              )}>
          ...
          <div className="flex gap-3 flex-wrap">
            {canManage && (
              <>
                <button
                  onClick={() => navigate(isRevenueRoute ? `/dashboard/revenue/investigations/${id}/edit` : `/dashboard/investigations/${id}/edit`)}
            ...
            {isClosed ? (
              <button
                ...
                className={clsx("text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg transition-all", isGeneratingPdf ? "bg-slate-700 cursor-not-allowed" : "bg-slate-800 hover:bg-slate-700")}
              >
                <Download size={20} />
              ...
              <button
                ...
                disabled={isGeneratingPdf}
                className={clsx("text-white px-6 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg transition-all", isGeneratingPdf ? "bg-slate-700 cursor-not-allowed" : "bg-red-600 hover:bg-red-500 shadow-red-900/50 hover:-translate-y-0.5")}
              >
                <CheckCircle size={20} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 pt-6 border-t border-slate-800 relative z-10">
          <div>
            ...
            <div className="text-white font-medium flex items-center gap-2">
              <span className={clsx("w-2 h-2 rounded-full", { 'bg-red-500': investigation.priority === 'Alta', 'bg-yellow-500': investigation.priority === 'Média', 'bg-emerald-500': investigation.priority === 'Baixa' })} />
            ...
            <div className="text-white font-medium flex items-center gap-2">
              ...
              <span className="truncate">
                {investigation.tipoAlvoInvestigacao === 'organizacao' || investigation.tipoAlvoInvestigacao === 'empresa'
                  ? `${investigation.nomeOrganizacaoInvestigada || 'Não informado'}${investigation.investigados?.length ? ` | ${investigation.investigados.map(i => i.nome || 'Não informado').join(', ')}` : ''}`
                  : (investigation.investigados?.map(i => i.nome || 'Não informado').join(', ') || investigation.nomeInvestigado || investigation.involved || 'Não informado')}
              </span>
            </div>
          </div>
          <div>
            ...
            <div className="text-white font-medium flex items-center gap-2">
              ...
              <span className="truncate">
                {investigation.investigados?.length
                  ? investigation.investigados.map(i => i.cpf || 'Não informado').join(', ')
          ...
          <div>
            ...
            <div className="text-white font-medium flex items-center gap-2">
              ...
              {new Date(investigation.createdAt).toLocaleDateString('pt-BR')}
            </div>
          </div>
          <div>
          ...
          {investigation.proofs && investigation.proofs.length > 0 ? (
            investigation.proofs.map((proof) => (
              <ProofCard
                key={proof.id}
                proof={proof}
                onClick={(p) => navigate(isRevenueRoute ? `/dashboard/revenue/investigations/${id}/proofs/${p.id}` : `/dashboard/investigations/${id}/proofs/${p.id}`)}
                onDelete={handleDeleteProof}
                onEdit={(p) => { setProofToEdit(p); setIsEditModalOpen(true); }}
                canEdit={!isClosed}
              />
            ))
          ) : (
      ...
      <AddProofModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSave={handleAddProof} />
      <EditProofModal
        isOpen={isEditModalOpen}
        onClose={() => { setIsEditModalOpen(false); setProofToEdit(null); }}
