...
const normalizeInvestigatedList = (value) => {
  ...
  const sanitized = value
    .map((item) => ({ nome: String(item?.nome || '').trim(), cpf: String(item?.cpf || '').trim() }))
...
const InvestigationCreate = () => {
    ...
    if (id) {
      const fetchData = async () => {
        try {
          ...
          setNotification({ type: 'error', message: 'Não foi possível carregar os dados da investigação.' });
        } finally {
          setLoading(false);
        }
      };
      ...
      if (categoryParam) setFormData(prev => ({ ...prev, category: categoryParam }));
      setLoading(false);
    }
  ...
  const handleInvestigadoChange = (index, field, value) => {
    setFormData((prev) => {
      const investigados = prev.investigados.map((item, i) => i === index ? { ...item, [field]: value } : item);
      const principal = investigados[0] || createEmptyInvestigated();
      return { ...prev, investigados, nomeInvestigado: principal.nome, cpfInvestigado: principal.cpf };
    });
  };

  const addInvestigado = () => setFormData(prev => ({ ...prev, investigados: [...prev.investigados, createEmptyInvestigated()] }));

  const removeInvestigado = (index) => {
    setFormData((prev) => {
      const investigados = prev.investigados.filter((_, i) => i !== index);
      ...
      return { ...prev, investigados: normalized, nomeInvestigado: principal.nome, cpfInvestigado: principal.cpf };
    });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const notif = { type: 'success', message: id ? 'Investigação atualizada com sucesso.' : 'Investigação criada com sucesso.' };
      const basePath = formData.category === 'financial' ? '/dashboard/revenue/investigations' : '/dashboard/investigations';
      navigate(targetId ? `${basePath}/${targetId}` : basePath.replace('/investigations', ''), targetId ? { state: { notification: notif } } : undefined);
    } catch (error) {
      setNotification({ type: 'error', message: error?.message || 'Não foi possível salvar a investigação.' });
    } finally {
    ...
    <div className="max-w-3xl mx-auto pb-10">
      {notification && (
        <div className={clsx(
          ...
          <button type="button" onClick={() => setNotification(null)} className="text-current/80 hover:text-current transition-colors" aria-label="Fechar aviso">
            <X size={18} />
          </button>
        </div>
      )}

      <button onClick={() => navigate(backLink)} className="mb-6 flex items-center gap-2 text-slate-400 hover:text-white transition-colors text-sm font-bold uppercase tracking-wider">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-xl">
        <div className="mb-8 border-b border-slate-800 pb-6">
          ...
          <p className="text-slate-400 mt-2">
            {id ? 'Atualize os dados da investigação.' : isFinancial ? 'Abra um novo inquérito para apurar crimes financeiros.' : 'Preencha os dados iniciais para abrir um novo inquérito.'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input type="text" name="title" value={formData.title} onChange={handleChange}
              ...
              placeholder="Ex: Operação Lava Jato" required />
          </div>
          ...
          <div>
            ...
            <textarea name="description" value={formData.description} onChange={handleChange} rows={5}
              ...
              placeholder="Detalhes iniciais do caso..." required />
          </div>

          <div className="border-t border-slate-800 pt-6 space-y-6">
            <div>
              <h3 className="text-sm font-bold text-white uppercase tracking-wider">Dados do Inquérito</h3>
              <p className="text-sm text-slate-400 mt-1">Essas informações alimentam o relatório final automaticamente quando a investigação for encerrada.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" name="delegaciaResponsavel" value={formData.delegaciaResponsavel} onChange={handleChange}
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-600 focus:border-federal-500 focus:ring-1 focus:ring-federal-500 transition-all outline-none"
              ...
              <div>
                ...
                <input type="text" name="nomeDelegado" value={formData.nomeDelegado} onChange={handleChange}
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-600 focus:border-federal-500 focus:ring-1 focus:ring-federal-500 transition-all outline-none"
          ...
          <div className="border-t border-slate-800 pt-6 space-y-6">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">Escolha se a investigação é contra pessoa, organização ou empresa e informe todos os investigados necessários.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">Tipo do Alvo</label>
                <select name="tipoAlvoInvestigacao" value={formData.tipoAlvoInvestigacao} onChange={handleChange}
              ...
              {isEntityTarget && (
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" name="nomeOrganizacaoInvestigada" value={formData.nomeOrganizacaoInvestigada} onChange={handleChange}
            ...
            <div>
              <div className="flex items-center justify-between gap-3 mb-3">
                ...
                <button type="button" onClick={addInvestigado}
                  ...
                  <Plus size={14} /> Adicionar Investigado
                </button>
              </div>
                ...
                {formData.investigados.map((investigado, index) => (
                  <div key={`investigado-${index}`} className="grid grid-cols-1 md:grid-cols-[1fr_220px_auto] gap-3">
                    <div className="relative">
                      ...
                      <input type="text" value={investigado.nome} onChange={(e) => handleInvestigadoChange(index, 'nome', e.target.value)}
                        className="w-full pl-12 pr-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-600 focus:border-federal-500 focus:ring-1 focus:ring-federal-500 transition-all outline-none"
                        placeholder={`Nome do investigado ${index + 1}`} />
                    </div>
                    <input type="text" value={investigado.cpf} onChange={(e) => handleInvestigadoChange(index, 'cpf', e.target.value)}
                      className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-600 focus:border-federal-500 focus:ring-1 focus:ring-federal-500 transition-all outline-none"
                      placeholder="CPF / Documento" />
                    <button type="button" onClick={() => removeInvestigado(index)}
                      className="px-4 py-3 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl transition-colors flex items-center justify-center" title="Remover investigado">
                      <Trash2 size={16} />
                    </button>
                  </div>
                ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="date" name="dataNascimento" value={formData.dataNascimento || ''} onChange={handleChange}
                  className={clsx("w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 focus:border-federal-500 focus:ring-1 focus:ring-federal-500 transition-all outline-none", !formData.dataNascimento && "text-slate-500")} />
              </div>
              <div>
                <label className="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">Telefone</label>
                <input type="text" name="telefoneInvestigado" value={formData.telefoneInvestigado} onChange={handleChange}
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-600 focus:border-federal-500 focus:ring-1 focus:ring-federal-500 transition-all outline-none"
                  placeholder="Telefone para contato" />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-slate-400 mb-2 uppercase tracking-wider">Endereço</label>
                <input type="text" name="enderecoInvestigado" value={formData.enderecoInvestigado} onChange={handleChange}
                  className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-600 focus:border-federal-500 focus:ring-1 focus:ring-federal-500 transition-all outline-none"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <input type="text" name="involved" value={formData.involved} onChange={handleChange}
                className="w-full px-4 py-3 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 placeholder-slate-600 focus:border-federal-500 focus:ring-1 focus:ring-federal-500 transition-all outline-none"
          ...
          <div className="pt-6 border-t border-slate-800 flex justify-end">
            <button type="submit" disabled={isSubmitting}
              className={clsx("text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-federal-900/50 transition-all", isSubmitting ? "bg-slate-700 cursor-not-allowed" : "bg-federal-600 hover:bg-federal-500 hover:-translate-y-0.5")}>
              <Save size={20} />
