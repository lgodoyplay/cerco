...
import { Shield, FileText, Upload, CheckCircle, AlertTriangle, ChevronRight, Send, Loader2, X } from 'lucide-react';
import { supabase } from '../../../lib/supabase';
import { useAuth } from '../../../context/AuthContext';

const RequestWeaponLicense = () => {
  const { user } = useAuth();
  ...
  const steps = [
    ...
    "Aguardar a análise inicial da FEDERAL EUFORIA (acompanhe pelo Discord/Email).",
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            ...
            Solicitação de Porte de Armas
          </h1>
          <p className="text-slate-400 mt-1">
            Preencha o formulário abaixo para iniciar o processo de legalização.
          </p>
        </div>
      </div>
      ...
      {step === 2 && (
        <div className="max-w-2xl mx-auto bg-slate-900/50 border border-slate-800 rounded-2xl p-6 md:p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {step === 3 && (
        <div className="max-w-md mx-auto bg-slate-900/50 border border-slate-800 rounded-2xl p-8 text-center animate-in zoom-in duration-300">
          <div className="w-16 h-16 bg-green-500/10 text-green-500 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle size={32} />
          </div>
          <h2 className="text-2xl font-bold text-white mb-2">Solicitação Enviada!</h2>
          <p className="text-slate-400 mb-8">
            Sua solicitação foi registrada com sucesso. A FEDERAL EUFORIA analisará seus dados e entrará em contato em breve.
          </p>
          <button
            onClick={() => {
              setStep(1);
...
export default RequestWeaponLicense;
