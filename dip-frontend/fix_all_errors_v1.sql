-- FIX ALL ERRORS (RPC + DATABASE COLUMNS)
-- Run this script in Supabase SQL Editor to fix:
-- 1. "column cursos_policiais.certificado_url does not exist"
-- 2. "create_user_command" RPC errors (400/404/42702)

-- PART 1: FIX DATABASE SCHEMA
-- Add the missing column that causes ProfilePage to crash
ALTER TABLE public.cursos_policiais 
...
-- Attempt to create policies (will fail safely if they exist, can be ignored)
DO $$
BEGIN
...
-- PART 2: FIX RPC FUNCTION (create_user_command)
-- Clean up all previous versions to avoid conflicts
...
-- Recreate the function with STANDARD parameter names (email, password)
-- Uses internal variables to prevent "ambiguous column" errors
