...
CREATE POLICY "Excluir investigações" ON public.investigacoes FOR DELETE USING (auth.role() = 'authenticated');
...
CREATE POLICY "Excluir provas" ON public.provas FOR DELETE USING (auth.role() = 'authenticated');
...
CREATE POLICY "Agentes podem deletar provas" ON storage.objects FOR DELETE USING (bucket_id = 'provas' AND auth.role() = 'authenticated');

-- ==============================================================================
-- PARTE X: ATUALIZAÇÃO PARA CÓDIGO FUNCIONAL E POLICIES PÚBLICAS
-- ==============================================================================

-- 1. Adicionar coluna codigo_funcional em profiles
ALTER TABLE public.profiles ADD COLUMN IF NOT EXISTS codigo_funcional TEXT UNIQUE;

-- 2. Garantir policies para busca pública de verificação
-- Permitir que qualquer um leia profiles onde o código funcional bate (para verificação)
-- Nota: Isso pode exigir desabilitar RLS ou criar uma policy complexa.
-- Uma abordagem segura é criar uma função SECURITY DEFINER para buscar por código.
...
-- 3. Garantir policies para busca pública de Presos e Procurados
-- Já existem policies para 'auth.role() = authenticated' e 'true' para contagem.
-- Precisamos liberar leitura de dados básicos para a página de pesquisa pública.

DROP POLICY IF EXISTS "Leitura pública de prisoes" ON public.prisoes;
CREATE POLICY "Leitura pública de prisoes" ON public.prisoes
  FOR SELECT USING (true);

DROP POLICY IF EXISTS "Leitura pública de procurados" ON public.procurados;
CREATE POLICY "Leitura pública de procurados" ON public.procurados
  FOR SELECT USING (true);
