...
DO $$
BEGIN
    ALTER TABLE public.profiles ALTER COLUMN role SET DEFAULT 'Agente DPF';
EXCEPTION
...
-- PERMITIR QUE TODOS CRIEM CURSOS (PARA TESTES)
create policy "Todos podem criar cursos" on public.cursos
  for insert with check (auth.role() = 'authenticated');

create policy "Todos podem editar cursos" on public.cursos
  for update using (auth.role() = 'authenticated');
  
create policy "Todos podem deletar cursos" on public.cursos
...
-- PERMITIR QUE TODOS ATRIBUAM CURSOS (PARA TESTES)
create policy "Todos podem atribuir cursos" on public.cursos_policiais
  for insert with check (auth.role() = 'authenticated');

create policy "Todos podem remover atribuições" on public.cursos_policiais
  for delete using (auth.role() = 'authenticated');
